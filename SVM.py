...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov  4 14:20:16 2019
...
scaler.fit(X_2Dsel)
X_2Dsel = scaler.transform(X_2Dsel)
...
output = "The best accuracy on the validation set is " + '{:.2f}'.format(
    bestAcc * 100) + "% \nFound with C=" + '{:.3f}'.format(bestParams[0]) + " and gamma="'{:.3f}'.format(bestParams[1])
print(output)
...
plt.title("SVM with RBF kernel (c=%.3f) (gamma=%.3f) \nTest points plotted" % (bestParams[0], bestParams[1]),
          fontsize=16)
plt.show()
...
X_trainVal, y_trainVal = np.concatenate((X_train, X_test)), np.concatenate((y_train, y_test))
k = 5
